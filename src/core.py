...
def compute_sentiment(utterance: str):
    nlp = pipeline("sentiment-analysis")
    result = nlp(utterance)[0]
...
if __name__ == '__main__':
    ...
    while True:
        ...
        Input = GivenCommand()
        talk("Computed sentiment is".format(compute_sentiment)) 
        ...
        elif 'poem' or 'smalltalk' or 'what do you think'  in Input:
            output = smalltalk(Input)
            #talk("Was that to your liking?")
            recipient = GivenCommand()
            #escaped_output = output.replace("\n","\\n")
            fields=[datetime.datetime.utcnow() , Input, output]
